...
export interface StructuredMessage {
  role: 'system' | 'user'
  content: string | StructuredContentPart[]
}
...
export interface StructuredTransport {
  ...
  complete(input: StructuredCompletionInput): Promise<ProviderCompletion<unknown>>
}
...
function mapStructuredHttpStatus(status: number, details: ProviderErrorDetails) {
...
function parseStructuredCompletion(value: unknown, observation: ProviderAttemptObservation): ProviderCompletion<unknown> {
...
export function createStructuredTransport(options: StructuredTransportOptions): StructuredTransport {
  return {
    ...
    async complete(input) {
      ...
      if (!response.ok) {
        ...
        const retryAfterMs = response.status === 429
          ...
          : undefined
        throw mapStructuredHttpStatus(response.status, errorDetails('confirmed', providerElapsedMs, {
      ...
      const providerElapsedMs = elapsedSince(startedAt, now)
      // Some compatible APIs return a terminal error envelope after HTTP 200.
      if (isRecord(payload) && isRecord(payload.error) && typeof payload.error.code === 'number') {
        throw mapStructuredHttpStatus(payload.error.code, errorDetails('confirmed', providerElapsedMs, {
          attemptObservations: [unknownAttemptObservation(attemptDiagnosticId, providerElapsedMs)],
        }))
      }
      ...
      const finishReason = (Array.isArray(message?.tool_calls) && message.tool_calls.length > 0)
        ...
        : rawFinishReason === 'stop' || rawFinishReason === 'length' || rawFinishReason === 'content_filter' || rawFinishReason === 'tool_calls'
          ? rawFinishReason
          : 'unknown'
      return parseStructuredCompletion(payload, {
