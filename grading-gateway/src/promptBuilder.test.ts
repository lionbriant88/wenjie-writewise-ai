...
describe('buildGradingPrompt', () => {
  it('contains JSON invariants, a complete minimum example, and untrusted data boundaries', () => {
    ...
    expect(prompt.system).toContain('relatedIssueKeys')
    expect(prompt.system).toContain('不超过 50 个且无重复')
    expect(prompt.system).toContain('满分时必须为空数组，扣分时必须至少关联一个原始 issueKey')
    expect(`${prompt.system}\n${prompt.user}`).toContain('"fullTextRevision"')
    expect(prompt.system).toContain('"logicIssues": []')
    ...
    expect(prompt.system).toContain('contextBefore/contextAfter 可为空字符串；非空时必须逐字唯一匹配正文并分别位于 originalText 前/后')
  })
})
