...
const minimumJsonExample = {
  dimensionScores: [{
    ...
    reason: '评分理由',
    evidence: '来自作文原文的证据',
    relatedIssueKeys: ['issue-1'],
  }],
  issues: [{ issueKey: 'issue-1', type: 'grammar', severity: 'low', originalText: '原句', suggestion: '修改句', explanation: '理由', evidenceCertainty: 'certain', requiresTeacherReview: false }],
  sentenceRevisions: [{ originalText: '原句', revisedText: '修改句', note: '说明', relatedIssueKeys: ['issue-1'], changeTypes: ['grammar'] }],
  expressionUpgrades: [],
  fullTextRevision: {
    correctedText: '完整纠错稿',
    improvedText: '完整提升稿',
    sentencePairs: [{ originalText: '原句', correctedText: '修改句', improvedText: '提升句', relatedIssueKeys: ['issue-1'], changeTypes: ['grammar'], explanation: '说明', requiresTeacherReview: false }],
    logicNotes: [],
    logicIssues: [],
  },
  recognitionWarnings: [],
...
export function buildGradingPrompt(request: GradingRequestV1): GradingPromptV1 {
  ...
  const system = [
    ...
    '每个 rubric dimensionId 必须恰好出现一次。score 必须是有限数，且不得超过提供的 maxScore。每个 dimensionScores item 必须有不超过 50 个且无重复的 relatedIssueKeys；满分时必须为空数组，扣分时必须至少关联一个原始 issueKey。',
    'reportedTotalScore 如提供也只是候选值；最终总分由系统按分项分求和并四舍五入为整数。',
    'issues、sentenceRevisions、expressionUpgrades 和 sentencePairs 的 originalText 必须逐字来自已确认正文。每个 issue 必须提供唯一 issueKey 和 evidenceCertainty；每个 revision/pair 必须提供非空且无重复的 relatedIssueKeys 与非空 changeTypes。fullTextRevision.logicIssues、legibilityIssues 和 recognitionWarnings 都必须存在。确认文本没有视觉歧义：recognitionWarnings 和 legibilityIssues 必须为空数组。',
