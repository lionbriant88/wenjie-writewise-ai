...
const sentencePairSchema = { type: 'object', additionalProperties: false, required: ['originalText', 'correctedText', 'improvedText', 'relatedIssueKeys', 'changeTypes', 'explanation', 'requiresTeacherReview'], properties: { originalText: { type: 'string' }, correctedText: { type: 'string' }, improvedText: { type: 'string' }, relatedIssueKeys: { type: 'array', items: { type: 'string' } }, changeTypes: { type: 'array', items: changeTypeSchema }, explanation: { type: 'string' }, requiresTeacherReview: { type: 'boolean' } } } as const
...
export const essayGradingSchema = {
  ...
  required: ['transcript', 'recognitionWarnings', 'printedTextExcluded', 'reportedTotalScore', 'dimensionScores', 'issues', 'sentenceRevisions', 'expressionUpgrades', 'fullTextRevision', 'legibilityIssues', 'overallComment', 'reviewReasons'],
  properties: {
    transcript: { type: 'string' }, recognitionWarnings: { type: 'array', maxItems: 50, items: recognitionWarningSchema }, printedTextExcluded: { type: 'boolean' }, reportedTotalScore: { type: 'number' }, dimensionScores: { type: 'array', items: dimensionScoreSchema }, issues: { type: 'array', maxItems: 100, items: issueSchema }, sentenceRevisions: { type: 'array', maxItems: 100, items: revisionSchema }, expressionUpgrades: { type: 'array', items: upgradeSchema },
    fullTextRevision: { type: 'object', additionalProperties: false, required: ['correctedText', 'improvedText', 'sentencePairs', 'logicNotes', 'logicIssues'], properties: { correctedText: { type: 'string' }, improvedText: { type: 'string' }, sentencePairs: { type: 'array', maxItems: 100, items: sentencePairSchema }, logicNotes: { type: 'array', items: logicNoteSchema }, logicIssues: { type: 'array', maxItems: 50, items: logicIssueSchema } } },
...
export function buildEssayGradingMessages(input: BuildEssayGradingMessagesInput): KimiMessage[] {
  ...
  return [{ role: 'system', content: [
    ...
    ...(hasConfirmedTranscript ? [] : [
      ...
      'A localizable important ambiguity belongs only in legibilityIssues; do not repeat it in recognitionWarnings or reviewReasons. Global, unlocalizable, or printed/student-boundary uncertainty may use recognitionWarnings with scope global_unreadable or printed_boundary. 局部可定位歧义只能写入 legibilityIssues，不能用 recognitionWarnings 表示。',
    ]),
