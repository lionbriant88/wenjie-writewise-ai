...
function text(value: unknown, maxLength = 50_000): string | null { if (typeof value !== 'string') return null; const trimmed = value.trim(); return trimmed && trimmed.length <= maxLength ? trimmed : null }
function quote(value: unknown, maxLength = 50_000): string | null { return typeof value === 'string' && value.length > 0 && value.length <= maxLength ? value : null }
function rawContext(value: unknown, maxLength = 50_000): string | null { return typeof value === 'string' && value.length <= maxLength ? value : null }
...
function parseRawSentenceRevisions(value: unknown): RawSentenceRevisionV1[] | null {
  ...
  for (const item of value) {
    ...
    const relatedIssueKeys = textArray(item.relatedIssueKeys, 100, 200)
    if (!originalText || !revisedText || !note || !relatedIssueKeys || new Set(relatedIssueKeys).size !== relatedIssueKeys.length || item.changeTypes.length === 0 || item.changeTypes.length > 20 || new Set(item.changeTypes).size !== item.changeTypes.length || !item.changeTypes.every((entry) => typeof entry === 'string' && CHANGE_TYPES.has(entry))) return null
...
function parseRawSentencePairs(value: unknown): RawSentencePairV1[] | null {
  ...
  for (const item of value) {
    ...
    const relatedIssueKeys = textArray(item.relatedIssueKeys, 100, 200)
    if (!originalText || !correctedText || !improvedText || !explanation || !relatedIssueKeys || new Set(relatedIssueKeys).size !== relatedIssueKeys.length || typeof item.requiresTeacherReview !== 'boolean' || item.changeTypes.length === 0 || item.changeTypes.length > 20 || new Set(item.changeTypes).size !== item.changeTypes.length || !item.changeTypes.every((entry) => typeof entry === 'string' && CHANGE_TYPES.has(entry))) return null
...
export function normalizeMultimodalResult(payload: unknown, context: MultimodalNormalizationContext): MultimodalNormalizationResult {
  if (!isRecord(payload) || Object.keys(payload).some((key) => ![
    'transcript', 'recognitionWarnings', 'printedTextExcluded', 'reportedTotalScore', 'dimensionScores',
  ...
  if (!request || !issues || !revisions || !upgrades || !pairs || !logicNotes || !logicIssues || !legibilityIssues || !rawScoresAreBounded(payload.dimensionScores, context) || !isRecord(payload.fullTextRevision) || typeof payload.fullTextRevision.correctedText !== 'string' || typeof payload.fullTextRevision.improvedText !== 'string' || logicNotes.some(({ quote }) => !exactUniqueTranscriptRange(transcript, quote)) || logicIssues.some((issue) => !hasOrderedLogicContext(transcript, issue))) return invalid()
  ...
  const dimensionScoreCandidates = request.task.rubric.dimensions.map((dimension) => {
    ...
    const evidence = score ? quote(score.evidence, 50_000) : null
    const relatedIssueKeys = score ? textArray(score.relatedIssueKeys, 100, 200) : null
