...
## 2026-09-13：部署方向改为腾讯云集中部署

- 用户在后续确认中选择“替代 Vercel + Supabase，集中部署到腾讯云”，拟租轻量应用服务器 2 核 4GB、70GB SSD、6Mbps、600GB/月流量。本节取代下方同日 Vercel + Supabase 选型，后续不再默认使用该托管组合。
- 本轮只更新选型并说明账号实名认证、活动套餐、系统/地域选择和备案准备；尚未购买云资源、创建实例、修改应用代码或部署。个人可购买主机，但教师工具的个人备案适用性仍需按真实业务和所在地核实。
- 账号隔离、任务/原图/结果持久化、后台队列、幂等与用量控制仍待实现。集中部署不改变 Kimi 直接多模态 v2、360000ms 超时、有界并发及未知结果不盲目重试的决策，也不意味着现有程序可以直接对外试用。

## 历史选型：2026-09-13 Vercel + Supabase（已被同日后续腾讯云决定取代）

- 用户明确选择 Vercel + Supabase，首批规模为 20–50 位教师；当前为个人主体、暂无营业执照。这是部署技术选型，尚未购买套餐、创建云项目、接入账号或发布网站/小程序。
- 建议职责为 Vercel 承载现有网页、业务接口及异步批改执行，Supabase 承载教师身份、PostgreSQL、私有图片和持久任务记录。具体队列执行、共享准入、预算预留与未知结果恢复机制仍需设计验证；不能把现有内存队列直接视为云端可靠队列。
- 当前 360000ms Kimi 超时、一次多模态 completion、外部 v2 合同和教师确认流程保持不变。上传与异步任务编排需要适配平台限制，不因选型恢复 OCR、压缩未经验证的原图或放开 Provider 并发。
- 后续应先验证合成材料下的账号隔离、私有上传、后台执行和重启/重放恢复，再验证教师所在地网络、手机及微信入口。托管地域、学生信息处理授权、免费额度、备份恢复与小程序主体/类目条件需在对外试用前落实。

## 2026-09-06：质量修复与免费教师试用准备

- 用户已批准修复本轮质量问题，后续先考虑免费小范围教师使用。修复在 `.worktrees/codex-ai-pipeline-optimization` 完成，尚未合并本目录 `main` 或部署。
- 已修复分类标签混入正文、裁切漏提示、可合理辨正确字迹误扣分的处理机制，以及 PDF 转换期间继续拍照/上传造成页数超限和迟到 URL 泄漏。独立语法/用词错误继续保留，未知扣分归属仍要求教师复核。内部 Provider Schema 升 v3，对外多模态 v2 合同和一次 Kimi 请求主流程不变。
- 修复后在原剩余额度内只复测 a04/b04/b05：标签已排除、两页裁边提示及缺失标记出现、原字迹误扣消失，原图核对完成；1 success、2 partial。仍有评分依据复核和个别语法过度纠正，不是所有模型质量问题清零，也不是同标准评分 A/B。
- 两轮累计 24/24 completion、18/18 tokenizer，估算 9.999592 元（约 10 元）；无未知预留。本次 3 次同身份重放均 0 新调用，Provider 并发 1。已用完原调用数额度，不因预算余额而继续调用。
- 最终本地验证：网站 84 文件/1333 项，Gateway 52 文件/1336 项；两端类型检查、网站 lint/build、共享计分运行时、策略 fixtures 和正常 Git 配置下 diff 检查通过。
- 此前真实验收和本轮均未实际调用摄像头；新增的是 `capture` 文件输入归属、页序、重复选择及并发上传测试。手机相机权限、取消、横竖屏和微信/iOS/Android 兼容性仍需真机验收。
- 建议先做邀请制网页版：账号与教师数据隔离、任务/图片/结果持久化、后台队列与持久幂等、免费额度、HTTPS/私有存储/备份恢复完成后，再邀请少量教师；支付与独立小程序可后置。需用户准备运营主体、云账号/域名、教师名单、月预算及数据保留/授权规则。详见 `docs/2026-09-06-quality-fixes-and-teacher-pilot.md`。

## 2026-09-06：上线准备只读检查与真实样本预检

- 本轮最新实现检查对象为 `.worktrees/codex-ai-pipeline-optimization` 的 `codex/ai-pipeline-optimization` 分支、提交 `ee9c50c`，其状态记录更新至 2026-09-04。主目录 `main` 的 `ddc05ce` 尚未合入该实现，下文主目录顶部“优化尚未实施”记录是该 checkout 的历史阶段，不能替代最新工作树。此次未合并或发布。
- 优化工作树本轮本地回归最终通过：网站 84 文件 / 1329 项，Gateway 50 文件 / 1291 项，两端 typecheck、网站 lint/build、共享计分运行时与策略 fixtures 均通过。首次同时默认高并行存在四项时序/超时失败，控制并行后无代码修改全过；不能表述为首轮零失败或真实模型质量验收。
- 用户指定桌面“应用文”“读后续写”“作文图片”进行最终测试；已本地核对 23 张作文图片，整理为 18 个评分目标，另有两份 DOCX 原题材料。新密钥仅存入优化工作树被 Git 忽略的专用本地环境文件；只读模型目录检查 HTTP 200 且包含 `kimi-k3`。
- 用户已回复“可以”，明确同意全部 18 个评分目标、最多 24 次 completion、另最多 18 次 token 估算请求和 20 元预算。真实有限验收 `smoke-20260906-a` 已完成：2 次 rubric＋18 篇初批＋1 次确认文本重批，共 21 completion、18 tokenizer，按返回 usage 估算 8.388968 元，未结算预留为 0；费用不是账户账单核销。
- 18 篇初批均返回正文和可用分数：3 success、15 partial、0 failed；1 次零图片确认文本重批 success。19 次同身份结果重放均响应一致且没有增加 completion/tokenizer；最大 Provider 并发 1，所有 finish_reason 为 stop。初批平均模型成本约 0.43 元/篇，平均 Provider 耗时约 62.83 秒，不能推广为全班吞吐或所有作文固定单价。
- 本轮没有达到“全部无问题”的放行条件：一篇手写分类标签混入正文、一份裁边图未提示行末缺失、一处可按正确读法理解的字迹仍产生卷面扣分；扣分依据和关联降级较多。仅修正一个词的重批由 14/15 变为 13/15，也留下评分稳定性问题。适合继续受控教师测试，不建议直接公开收费。未修改生产策略来压低复核率。
- 真实验收通过生产 Gateway/Kimi Provider 和网站结果投影、领域适配器执行；完整结果在本机临时核对页逐篇视觉检查，19 份最终结果的全文和引用一致性检查通过。本轮没有重新走完整网站按钮交互，不覆盖小程序、支付或班级总结；也不替代 40＋40 A/B、100 次 soak、30 篇吞吐和图片变体实验。
- 当前商业阻塞已核实：任务/结果依赖 React 内存，缺教师身份/租户边界、持久后台队列与跨实例 registry、支付权益和退款对账；班级总结默认 fake 原型，微信小程序只有计划。租服务器本身不能补齐这些能力，本轮没有实现商业基础设施。
- 完整检查与官方来源见 `docs/2026-09-06-prelaunch-readiness-review.md`。本轮检查继续遵循 Kimi 直接多模态 v2、有界并发、稳定幂等身份及原图输入，没有启动或恢复 OCR。
