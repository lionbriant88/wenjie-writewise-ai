...
const processingStatuses = new Set<EssayStatus>([
  'pending_ocr', 'ocr_running', 'pending_grading', 'grading',
])
...
export function getProgressQueueStats(essays: Essay[]): ProgressQueueStats {
  ...
  const processing = essays.filter((essay) => processingStatuses.has(essay.status)).length
  const reviewNeeded = essays.filter((essay) => (
    essay.status === 'needs_review' || essay.status === 'grading_ready'
  )).length
  ...
  return {
    ...
    processable: essays.filter((essay) => isProcessableEssayStatus(essay.status)).length,
  }
}

export function filterEssaysByProgressTab(essays: Essay[], tab: ProgressQueueTab): Essay[] {
  if (tab === 'processing') {
    return essays.filter((essay) => processingStatuses.has(essay.status))
  }

  if (tab === 'review') {
    return essays.filter((essay) => essay.status === 'needs_review' || essay.status === 'grading_ready')
  }
